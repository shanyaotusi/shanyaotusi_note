...
// 释放堆
heap freeHeap(heap h, void (*freeElem)(Elemtype)){
	if(h){
...
int getElemlocation(heap h, Elemtype e, int (*compair)(Elemtype, Elemtype), int (*prior)(Elemtype, Elemtype)){
	if(!h || !e) return -1;
	for(int i = 1;i<=h->len;++i){
		if(compair(h->elem[i], e) == 1)
			return i;
	}
	return -1;
}
void deleteHeapElem(heap h, Elemtype e,int (compair)(Elemtype, Elemtype), int (*prior)(Elemtype, Elemtype)){
	if(!h) return;
	int i = getElemlocation(h, e, compair, prior);
	if(i == -1) return;
	// 将要删除的节点与最后一个节点交换
	swapElem(&h->elem[i], &h->elem[h->len]);
	h->len--;
	// 调整堆
	heapDrop(h, i, prior);
}
void printHeap(heap h, void (*print)(Elemtype)){
	if(!h) return;
	printf("heap: \n");
	for(int i = 1;i<=h->len;++i){
		print(h->elem[i]);
